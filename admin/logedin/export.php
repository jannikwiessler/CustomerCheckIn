...
try {
    ...
    try {
        ...
        try {
            if (!$statement->prepare('SELECT restaurants.restaurant_name, customers.first_name, customers.last_name, customers.checkin_time, customers.checkout_time, customers.email, customers.tel, customers.address, customers.zip_code, customers.city FROM restaurants RIGHT JOIN customers ON customers.restaurant_id = restaurants.id WHERE restaurants.id = ? AND ((customers.checkout_time IS NULL AND customers.checkin_time >= ?) OR customers.checkout_time >= ?) AND (customers.checkin_time <= ? + INTERVAL 1 DAY) ORDER BY customers.checkin_time;')) {
                throw new Exception($statement->error);
            }

            $statement->bind_param('isss', $restaurantId, $_POST['from'], $_POST['from'], $_POST['to']);
            $statement->execute();
